...
void getValidPassword(char *buffer, FILE *errorFile);
...
int main() // compile with: gcc -pedantic -Wall -Wextra -Werror defense.c -o defense
{
    ...
    do
    {
        getValidPassword(password1, errorFile);
	printf("After first password check!\n");
        getValidPassword(password2, errorFile);
	printf("After second password check!\n");
...
void getValidPassword(char *buffer, FILE *errorFile)
{
    ...
    do
    {
        ...
        scanf("%s", buffer);
        valid = match(buffer, "^[[:alpha:]].{7,32}$", 1, errorFile);
	printf("Valid -->%d\n", valid);
    } while (valid == 0);
}
